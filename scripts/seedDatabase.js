...
async function seedDatabase() {
  try {
    ...
    const [student1Result] = await db.execute(
      ...
      ['student@school.com', studentPassword, 'Ali bin Ahmad', 'student', 4]
    );
    const student1Id = student1Result.insertId;

    const [student2Result] = await db.execute(
      'INSERT INTO users (email, password, full_name, role, primary_level) VALUES (?, ?, ?, ?, ?)',
      ['student2@school.com', studentPassword, 'Siti Nurhaliza', 'student', 4]
    );
    const student2Id = student2Result.insertId;

    const [student3Result] = await db.execute(
      'INSERT INTO users (email, password, full_name, role, primary_level) VALUES (?, ?, ?, ?, ?)',
      ['student3@school.com', studentPassword, 'Muhammad Hafiz', 'student', 5]
    );
    ...
    const [course1Result] = await db.execute(
      ...
      ['Mathematics (Primary 4)', 'Basic mathematics for Primary 4: addition, subtraction, multiplication, and division', teacher1Id, 4]
    );
    const course1Id = course1Result.insertId;

    const [course2Result] = await db.execute(
      'INSERT INTO courses (title, description, teacher_id, primary_level) VALUES (?, ?, ?, ?)',
      ['English Language (Primary 4)', 'English: grammar, vocabulary, and reading comprehension', teacher1Id, 4]
    );
    const course2Id = course2Result.insertId;

    const [course3Result] = await db.execute(
      'INSERT INTO courses (title, description, teacher_id, primary_level) VALUES (?, ?, ?, ?)',
      ['Science (Primary 4)', 'Science: plants, animals, and the environment', teacher2Id, 4]
    );
    const course3Id = course3Result.insertId;

    const [course4Result] = await db.execute(
      'INSERT INTO courses (title, description, teacher_id, primary_level) VALUES (?, ?, ?, ?)',
      ['Bahasa Melayu (Primary 5)', 'Malay language for Primary 5', teacher1Id, 5]
    );
    const course4Id = course4Result.insertId;
    console.log('✓ Courses created');

    // Enroll students in courses (levels match student primary_level)
    ...
    await db.execute('INSERT INTO enrollments (student_id, course_id) VALUES (?, ?)', [student3Id, course4Id]);
