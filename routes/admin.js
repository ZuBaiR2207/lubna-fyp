...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    const [recentUsers] = await db.execute(`
      SELECT id, email, full_name, role, primary_level, created_at 
      FROM users 
...
router.post('/users/create', async (req, res) => {
  try {
    const { email, password, full_name, role, primary_level } = req.body;
    ...
    if (role === 'student' && (primary_level === '' || primary_level == null)) {
      return res.render('admin/createUser', {
        user: req.session.user,
        error: 'Please select a primary level (Primary 1–5) for students'
      });
    }
    ...
    await userModel.create({ email, password, full_name, role, primary_level });
...
router.post('/users/:id/edit', async (req, res) => {
  try {
    const { email, full_name, role, primary_level } = req.body;
    if (role === 'student' && (primary_level === '' || primary_level == null)) {
      const editUser = await userModel.findById(req.params.id);
      return res.render('admin/editUser', {
        user: req.session.user,
        editUser: { ...editUser, email, full_name, role },
        error: 'Please select a primary level for students'
      });
    }
    await userModel.updateUser(req.params.id, { email, full_name, role, primary_level });
    res.redirect('/admin/users');
  } catch (error) {
