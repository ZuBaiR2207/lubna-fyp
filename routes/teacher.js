...
router.post('/courses/create', async (req, res) => {
  try {
    const { title, description, primary_level } = req.body;
    if (!title) {
    ...
    if (primary_level === '' || primary_level == null) {
      return res.render('teacher/createCourse', {
        user: req.session.user,
        error: 'Please select which primary level this course is for (Primary 1–5)'
      });
    }
...
router.get('/courses/:id', async (req, res) => {
  try {
    ...
    let allStudents;
    if (course.primary_level != null && course.primary_level !== '') {
      allStudents = await userModel.getStudentsByPrimaryLevel(Number(course.primary_level));
    } else {
      allStudents = await userModel.getStudents();
    }
